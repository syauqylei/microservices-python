...
class ListingsHandler(BaseHandler):
    ...
    def get(self):
        ...
        try:
            ...
            self.write_json(
                {"result": False, "errors": "invalid page_num"}, status_code=400
            )
            return
        ...
        try:
            ...
            self.write_json(
                {"result": False, "errors": "invalid page_size"}, status_code=400
            )
            return
        ...
        if user_id is not None:
            try:
                ...
                self.write_json(
                    {"result": False, "errors": "invalid user_id"}, status_code=400
                )
                return
    ...
    def post(self):
        ...
        if cursor.lastrowid is None:
            self.write_json(
                {"result": False, "errors": ["Error while adding listing to db"]},
                status_code=500,
            )
            return
...
if __name__ == "__main__":
    ...
    logging.info(
        "Starting listing service. PORT: {}, DEBUG: {}".format(
